...
def prune_sort_corr_angles(arr_1, arr_2, kind='euclidean'):
    ...
    # Calculate pairwise circular distances using cdist
    return distances.mean(), arr1_pruned, arr2_pruned

def extract_angle_arrays(data):
    angle_corrs = {}
...
def RSA_from_angles(angles_per_img):
    rdm_angles_per_img = {}
    for stim in angles_per_img:
        ...
        for idx1, subj1 in enumerate(angles_per_img[stim]):
            for idx2, subj2 in enumerate(angles_per_img[stim]):
                #print(angle_corrs[stim][subj1])
                corrs, _, _ =  prune_sort_corr_angles(np.abs(angles_per_img[stim][subj1]), np.abs(angles_per_img[stim][subj2]))
                RDMs[idx1, idx2] = corrs
                #print(np.abs(angle_corrs[stim][subj1]))
                #print(np.abs(angle_corrs[stim][subj2]))
